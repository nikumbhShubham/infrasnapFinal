...
//   // Inside UploadMediaPage
// useEffect(() => {
//   if (predictions) {
//     navigate("/reportPage", { state: { predictions } });
//   }
...
//   const colors = ['#FF5733', '#33FF57', '#3357FF', '#F1C40F', '#9B59B6']; // Define colors for each sub-stage

//   const renderSubStageProgressBar = () => {
//     if (!predictions || !subStageProgress[predictions.stage]) return null;

//     const subStages = Object.entries(subStageProgress[predictions.stage]);
//     let cumulativeWidth = 0; // To keep track of cumulative width for each sub-stage

//     return (
//       <div className="w-full bg-gray-300 rounded-full h-4 mb-4 flex">
//         {subStages.map(([subStage, progress], index) => {
//           //const width = (progress / 5).toFixed(2); // Divide by 5 to get percentage out of 100%
//           const width = progress;
//           cumulativeWidth += parseFloat(width); // Update cumulative width
...
//               }}
//             >
//               {/* Optional: Display sub-stage name and progress */}

//               <span className="text-xs font-semibold text-black">{subStage}: {width}%</span>
//             </div>
//           );
//         })}
//         {/* Optional: Display total percentage */}
//         <div className="absolute right-0 top-0 text-xs font-semibold text-blue-600">{cumulativeWidth.toFixed(2)}%</div>
//       </div>
//     );
//   };
...
const UploadMediaPage = () => {
  ...
  useEffect(() => {
    if (mediaType === "video" && predictions) {
      navigate("/reportPage", { state: { predictions } });
    }
  }, [mediaType, predictions, navigate]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-indigo-50 to-indigo-100 p-10">
      {!activeFolder ? renderUploadSection() : <div>Folder View</div>}
    </div>
  );
};
