...
const FoundationDashboard = () => {
  const [allFoundationImages, setAllFoundationImages] = useState([]);
  ...
  useEffect(() => {
    const fetchImages = async () => {
      const basePath = `${user.uid}/${site.id}/Foundation` ;
      ...
      try {
        ...
        setAllFoundationImages(allImages);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-100 to-indigo-50 p-8">
      ...
      <header className="mb-12">
        <h1 className="text-5xl font-bold text-indigo-700 text-center mb-4">Foundation Dashboard</h1>
      ...
      <section className="mb-16">
        ...
        <Slider {...sliderSettings}>
          {allFoundationImages.map((url, index) => (
